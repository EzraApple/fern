...
async function getCommentsByRepo({
  repo,
  prNumber
}) {
  try {
...
async function getReviewCommentsByRepo({
  repo,
  prNumber
}) {
  try {
    ...
    return data.map((comment) => ({
      ...
      startLine: comment.start_line ?? void 0,
...
async function addInlineCommentByRepo({
  repo,
  prNumber,
  path,
  line,
  ...
  return {
    ...
    path: data.path,
    line: data.line ?? void 0,
    startLine: data.start_line ?? void 0,
...
// src/.opencode/tool/github.ts
var get_pr_review_comments = tool({
  ...
  async execute(args) {
    try {
      const result = await withToolTimeout(
        (void 0)({ ...args, private: args.isPrivate }),
        TOOL_TIMEOUT_MS,
        "github_create_repo"
      );
...
var fork_repo = tool({
  ...
  async execute(args) {
    try {
      const result = await withToolTimeout(
        (void 0)(args.repo, { org: args.org, name: args.name }),
        TOOL_TIMEOUT_MS,
        "github_fork_repo"
      );
  ...
  async execute(args) {
    try {
      const pr = await withToolTimeout(
        (void 0)({ repo: args.repo, prNumber: args.prNumber }),
        TOOL_TIMEOUT_MS,
        "github_get_pr"
      );
