...
// src/.opencode/tool/repo.ts
import { tool } from "@opencode-ai/plugin";

// src/services/integrations/git.ts
...
var DEFAULT_GITHUB_ORG = "replohq";
...
// node_modules/.pnpm/@octokit+plugin-retry@8.0.3_@octokit+core@7.0.6/node_modules/@octokit/plugin-retry/dist-bundle/index.js
...
// node_modules/.pnpm/@octokit+auth-app@8.1.2/node_modules/@octokit/auth-app/dist-node/index.js
...
async function get2(cache2, options) {
...
async function set2(cache2, options, data) {
...
var ConfigSchema = external_exports.object({
  opencode: external_exports.object({
    apiKey: external_exports.string().min(1, "OPENAI_API_KEY is required")
  }),
  github: external_exports.object({
    token: external_exports.string().optional(),
    appId: external_exports.string().optional(),
    appPrivateKey: external_exports.string().optional(),
    appInstallationId: external_exports.string().optional()
  }),
...
var EMAIL_USER_CACHE_TTL_SECONDS = 604800;
var EMAIL_NOT_FOUND_SENTINEL = "__NOT_FOUND__";
...
async function getRepoFromPath(repoPath) {
  try {
...
async function createPR(input, repoPath) {
...
// src/services/integrations/git.ts
var GITHUB_ORG = process.env.GITHUB_ORG ?? DEFAULT_GITHUB_ORG;
var CACHE_DIR = join(homedir(), ".replee-cache", "repos");
...
function getBotEmail(workspace) {
  try {
    return execSync("git config user.email", { cwd: workspace, encoding: "utf-8", timeout: 1e4 }).trim() || null;
  } catch {
    return null;
  }
}
function getBranchCreatorEmail(workspace) {
  try {
    return execSync(
      `git log --format='%ae' --reverse HEAD 2>/dev/null | head -1`,
      { cwd: workspace, encoding: "utf-8", timeout: 1e4 }
    ).trim() || null;
  } catch {
    return null;
  }
}
function isOnBotCreatedBranch(workspace) {
...
function validateCommand(workspace, command) {
...
function detectPackageManager(workspace) {
  ...
  const pnpmCheck = spawnSync("which", ["pnpm"], { encoding: "utf-8", timeout: 5e3 });
...
function setupSSHKey() {
  ...
  try {
    mkdirSync(sshDir, { recursive: true, mode: 448 });
    const normalizedKey = sshKey.replace(/\\n/g, "\n").trim() + "\n";
    writeFileSync(keyPath, normalizedKey, { mode: 384 });
...
`;
    ...
    if (!existingConfig.includes("replee_agent")) {
      writeFileSync(configPath, existingConfig + configContent, { mode: 384 });
    }
...
function findGuidelineFiles(dir, maxDepth = 3, currentDepth = 0) {
...
function readCodingGuidelines(workspace) {
...
function readAllCodingGuidelines(workspace) {
...
function setupWorkspace({ repo, branchName }) {
  ...
  if (existsSync(join(cachedRepo, ".git"))) {
    const fetchResult = spawnSync("git", ["fetch", "--depth", "1", "origin", "main"], { cwd: cachedRepo, encoding: "utf-8", timeout: 3e4 });
    if (fetchResult.status === 0 && !fetchResult.signal) {
      spawnSync("git", ["reset", "--hard", "origin/main"], { cwd: cachedRepo, encoding: "utf-8", timeout: 1e4 });
    }
    cpSync(cachedRepo, workDir, { recursive: true });
  } else {
    const cloneResult = spawnSync("git", [...cloneArgs, cachedRepo], { encoding: "utf-8", timeout: 6e4 });
  ...
  spawnSync("git", ["fetch", "origin", branchName], { cwd: workDir, encoding: "utf-8", timeout: 3e4 });
  const branchResult = spawnSync("git", ["checkout", "-b", branchName], { cwd: workDir, encoding: "utf-8", timeout: 1e4 });
  if (branchResult.status !== 0) {
    spawnSync("git", ["checkout", branchName], { cwd: workDir, encoding: "utf-8", timeout: 1e4 });
    spawnSync("git", ["pull", "origin", branchName, "--ff-only"], { cwd: workDir, encoding: "utf-8", timeout: 3e4 });
  }
...
function runGitCommand(workspace, command) {
  const fullCommand = command.startsWith("git ") ? command : `git ${command}`;
  validateCommand(workspace, fullCommand);
  return execSync(fullCommand, { cwd: workspace, encoding: "utf-8", stdio: "pipe", timeout: 3e4 });
}
function commitAndPush({ workspace, message, skipLint, coAuthor }) {
  runGitCommand(workspace, "add -A");
  const status = execSync("git status --porcelain", { cwd: workspace, encoding: "utf-8", timeout: 1e4 });
  if (!status.trim()) {
  ...
  if (!skipLint && existsSync(join(workspace, "package.json"))) {
    ...
    try {
      execSync(lintCmd, { cwd: workspace, stdio: "pipe", encoding: "utf-8", timeout: 6e4 });
    } catch {
      try {
        execSync(lintFixCmd, { cwd: workspace, stdio: "pipe", encoding: "utf-8", timeout: 6e4 });
        runGitCommand(workspace, "add -A");
        execSync(lintCmd, { cwd: workspace, stdio: "pipe", encoding: "utf-8", timeout: 6e4 });
      } catch (fixError) {
        const lintOutput = fixError instanceof Error ? fixError.stderr || fixError.stdout || fixError.message : String(fixError);
        return { success: false, message: "Lint failed", error: "Lint failed. Auto-fix was attempted but some errors remain.", lintOutput: lintOutput.slice(0, 2e3) };
      }
    }
  }
...
Co-authored-by: ${coAuthor.name} <${coAuthor.email}>`;
  ...
  const remoteUrl = execSync("git remote get-url origin", { cwd: workspace, encoding: "utf-8", timeout: 1e4 }).trim();
  const branch = execSync("git branch --show-current", { cwd: workspace, encoding: "utf-8", timeout: 1e4 }).trim();
  return { success: true, message: "Changes committed and pushed", branch, remoteUrl };
}
async function createPR2({ workspace, title, body, draft }) {
  try {
    const branch = execSync("git branch --show-current", { cwd: workspace, encoding: "utf-8", timeout: 1e4 }).trim();
    const pr = await createPR({ title, body, branch, draft }, workspace);
...
async function addCommentToPR({ workspace, body }) {
  const branch = execSync("git branch --show-current", { cwd: workspace, encoding: "utf-8", timeout: 1e4 }).trim();
  const remoteUrl = execSync("git remote get-url origin", { cwd: workspace, encoding: "utf-8", timeout: 1e4 }).trim();
  ...
  await createIssueComment({ repo, issueNumber: prNumber, body });
}
function cleanupWorkspace(workspace) {
  rmSync(workspace, { recursive: true, force: true });
  return { success: true, message: `Workspace ${workspace} cleaned up` };
}
async function addPRReviewers({ repo, prNumber, maxReviewers = 3 }) {
  try {
    const octokit = getOctokit();
    const [owner, repoName] = repo.split("/");
    if (!owner || !repoName) {
    ...
    const contributorCounts = /* @__PURE__ */ new Map();
    ...
    const reviewers = [...contributorCounts.entries()].sort((a, b) => b[1].count - a[1].count).slice(0, maxReviewers).map(([, data]) => data.name);
...
async function addReviewersWithWorkspace({ workspace, prNumber, files, maxReviewers = 2 }) {
  try {
    const octokit = getOctokit();
    let selfUsername = "";
    try {
    ...
    for (const file of files) {
      try {
        const output = execSync(`git log -10 --format="%aE" -- "${file}"`, { cwd: workspace, encoding: "utf-8", timeout: 1e4 });
...
async function checkoutPR({ repo, prNumber }) {
  ...
  const fetchResult = spawnSync("git", ["fetch", "--depth", "1", "origin", prBranch], { cwd: workDir, encoding: "utf-8", timeout: 6e4 });
  if (fetchResult.status !== 0) {
    return { success: false, error: `Failed to fetch branch ${prBranch}: ${fetchResult.stderr}` };
  }
  const checkoutResult = spawnSync("git", ["checkout", prBranch], { cwd: workDir, encoding: "utf-8", timeout: 1e4 });
  if (checkoutResult.status !== 0 || checkoutResult.signal) {
    const trackResult = spawnSync("git", ["checkout", "-b", prBranch, `origin/${prBranch}`], { cwd: workDir, encoding: "utf-8", timeout: 1e4 });
    if (trackResult.status !== 0 || trackResult.signal) {
      const errorMsg = trackResult.signal ? `Checkout timed out (signal: ${trackResult.signal})` : `Failed to checkout branch ${prBranch}: ${checkoutResult.stderr}`;
      return { success: false, error: errorMsg };
    }
  }
  spawnSync("git", ["config", "user.email", "replee@replo.app"], { cwd: workDir, encoding: "utf-8", timeout: 5e3 });
  spawnSync("git", ["config", "user.name", "Replee"], { cwd: workDir, encoding: "utf-8", timeout: 5e3 });
  spawnSync("git", ["branch", "--set-upstream-to", `origin/${prBranch}`], { cwd: workDir, encoding: "utf-8", timeout: 5e3 });
  ...
  const ciEnv = {
    ...
    ADBLOCK: "true",
    HUSKY: "0"
    // Disable husky hooks during install
  };
  let actualPm = detectedPm;
  const pmCheck = spawnSync("which", [detectedPm], { encoding: "utf-8", timeout: 5e3 });
  if (pmCheck.status !== 0) {
    console.log(`[git] ${detectedPm} not found in PATH, falling back to npm`);
    actualPm = "npm";
  }
  const CHECKOUT_INSTALL_TIMEOUT_MS = 6e4;
  function getInstallCmd(pm, frozen) {
    switch (pm) {
      case "pnpm":
...
var REPO_LIST_CACHE_TTL_SECONDS = 300;
var LIST_REPOS_TIMEOUT_MS = 15e3;
async function listRepos({ search, limit = 20 } = {}) {
    ...
    try {
      ...
      if (search) {
        ...
        repos = data.items.map((repo) => ({
          ...
          url: repo.html_url,
          pushedAt: repo.pushed_at ?? void 0
        }));
      } else {
        const { data } = await withTimeout(
    ...
    try {
      const result = await withToolTimeout(
        new Promise((resolve, reject) => {
          try {
            resolve(setupWorkspace({ repo: args.repo, branchName: args.branchName }));
          } catch (err) {
            reject(err);
          }
        }),
...
var run_git = tool({
  ...
  async execute(args) {
    try {
      const output = runGitCommand(args.workspace, args.command);
...
var commit_and_push = tool({
  ...
  async execute(args) {
    try {
      const resolvedEmail = args.coAuthorGitHub ? `${args.coAuthorGitHub}@users.noreply.github.com` : void 0;
      const coAuthor = args.coAuthorName && resolvedEmail ? { name: args.coAuthorName, email: resolvedEmail } : void 0;
  ...
  async execute(args) {
    try {
      ...
      if (result.success && result.prUrl) {
        try {
          await addCommentToPR({ workspace: args.workspace, body: "@claude review" });
        } catch {
        }
      }
      return JSON.stringify(result, null, 2);
...
var cleanup_workspace = tool({
  ...
  async execute(args) {
    try {
      const result = cleanupWorkspace(args.workspace);
      return JSON.stringify(result, null, 2);
    } catch (error) {
      return JSON.stringify({ success: false, error: String(error) });
    }
...
var checkout_pr = tool({
  ...
  async execute(args) {
    try {
      const result = await withToolTimeout(
        checkoutPR({ repo: args.repo, prNumber: args.prNumber }),
...
var list_repos = tool({
  ...
  async execute(args) {
    try {
      const result = await withToolTimeout(
        listRepos({ search: args.search, limit: args.limit }),
        TOOL_TIMEOUT_MS,
        "repo_list_repos"
      );
      return JSON.stringify(result, null, 2);
